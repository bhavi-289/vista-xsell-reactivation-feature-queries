...
orders_today as (
    select 
    customer_id, city_id, order_id, order_received_timestamp, promo_code, day as anchor_date,
    basket_amount, promo_amount, net_basket_amount_with_tax,
    row_number() over (partition by customer_id order by order_received_timestamp) as day_order_rn
    from now_prod_dwh.orders o
    join dev_bi.quik_merchants as m 
    on o.merchant_id = m.merchant_id 
    where
    ...
    and city_id IN (1, 2)
    and order_type='shops' 
),
orders_after_today as (
    select 
    customer_id,
    count(distinct order_id) as num_orders_after_reactivation,
    sum(basket_amount) as total_basket_amount_after_reactivation,
    sum(promo_amount) as total_promo_amount_after_reactivation,
    sum(net_basket_amount_with_tax) as total_user_spend_after_reactivation
    from now_prod_dwh.orders o
    join dev_bi.quik_merchants as m 
    on o.merchant_id = m.merchant_id 
    where
...
sa_users as (
  ...
  where 
    a.day = date '{anchor_date}' --(select max(day) from sa_prod_agg.customer_status_details where day >= current_date - interval '2' day)
...
food_users as (
    ...
    where 
        day = date '{anchor_date}' --(select max(day) from now_prod_agg.customer_Status_details where day >= current_date - interval '2' day)
...
ot.basket_amount,
ot.promo_amount,
ot.net_basket_amount_with_tax,

oa.num_orders_after_reactivation,
oa.total_basket_amount_after_reactivation,
oa.total_promo_amount_after_reactivation,
oa.total_user_spend_after_reactivation,

-- sa.quik_reactivated_user,
...
from orders_today ot
left join
orders_after_today oa
on ot.customer_id = oa.customer_id
left join
sa_gp sa
on ot.customer_id = sa.customer_id
left join 
cplus_users
on ot.customer_id = cplus_users.customer_id
left join 
nationality
on ot.customer_id = nationality.customer_id